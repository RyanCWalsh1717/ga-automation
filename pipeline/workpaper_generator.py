...
def _write_debt_service_workpaper(wb, engine_result):
    ...
    for i, loan in enumerate(loans):
        ...
        if isinstance(pmt, dict):
            p_amt = pmt.get('principal', 0) or 0
            i_amt = pmt.get('interest', 0) or 0
            t_amt = pmt.get('taxes', 0) or 0
            ins_amt = pmt.get('insurance', 0) or 0
            r_amt = pmt.get('reserves', 0) or 0
            total = pmt.get('total_payment_due', 0) or (p_amt + i_amt + t_amt + ins_amt + r_amt)
        else:
