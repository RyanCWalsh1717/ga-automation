...
# Reference codes the pipeline stamps on every JE it generates (see the
# 'reference' field on je_lines dicts throughout this module and management_fee.py).
...
def _is_pipeline_txn(txn) -> bool:
    """
    True if a GL transaction was generated by THIS pipeline (an earlier run's
    accrual, reversal, management fee, RE tax/insurance amortization, or
    prepaid release), based on its description/remarks/reference — NOT its
    Yardi transaction control code.

    Why not use control code ('J' = Journal): every JE this pipeline posts is
    a Journal entry in Yardi, but so is any manual JE JLL (or anyone else)
    posts by hand — Yardi's control code reflects *how* an entry was keyed in,
    not *who* keyed it or *why*. Guards that treat "not J" as "real activity
    already posted" are blind to a JLL-posted manual JE that happens to
    accomplish the same thing the pipeline would have generated (e.g. their
    own management fee accrual, or their own RE tax deferral) — the pipeline
    would post a duplicate on top of it. This helper instead recognizes the
    pipeline's own output specifically, by the "Accrual {period} —" prefix
    and reference codes every JE-generating function in this codebase uses
    (confirmed present on every layer: Nexus, proration, historical, RE tax,
    insurance, bonus, management fee, catch-up, prepaid release).
    """
    if txn is None:
        return False
    ref = str(getattr(txn, 'reference', '') or '').strip().upper()
    if ref in _PIPELINE_JE_REFERENCES:
        return True
    text = f"{getattr(txn, 'description', '') or ''} {getattr(txn, 'remarks', '') or ''}"
    return 'accrual ' in text.lower()


def _real_net_change(gl_acct) -> float:
    """
    Net change on a GLAccount EXCLUDING the pipeline's own transactions
    (see _is_pipeline_txn) — i.e. genuine external activity, whether posted
    as a Charge/Receipt/Payable/Check OR as a manual Journal Entry by JLL or
    anyone else. Use this (not a J-vs-non-J split) to detect whether a real,
    non-pipeline entry already covers this period's accrual/release — the
    correct signal is "did the pipeline itself already post this," not
    "was this posted via Yardi's Journal Entry screen."
    """
    if gl_acct is None:
        return 0.0
    txns = getattr(gl_acct, 'transactions', [])
...
def detect_insurance_amortization(
    ...
    # 2. Find which insurance expense accounts already have real (non-pipeline)
    # expense activity this period. Using _real_net_change (not raw net_change)
    # excludes the pipeline's own prior-month auto-reversal credit, which
    # otherwise could mask a same-period JLL-posted debit in the net (e.g. a
    # $19K pipeline reversal credit + a $19K JLL debit nets to ~$0, wrongly
    # reading as "not yet posted" even though JLL already handled it).
    already_posted: set = set()
    for acct in (gl_data.accounts if hasattr(gl_data, 'accounts') else []):
        code = str(acct.account_code).strip()
        if code in _INSURANCE_EXPENSE_ACCTS and _real_net_change(acct) > 0.01:
...
def detect_retax_amortization(
    ...
    # Real (non-pipeline) net for suppression guards — the pipeline's own prior
    # deferral/release JEs should not trigger the "already posted" guard, but a
    # JLL-posted manual JE for the same purpose should (control code alone
    # can't tell a JLL JE apart from the pipeline's own — see _is_pipeline_txn).
    def _kpc_net(txns: list) -> float:
        return sum(
            float(t.debit or 0) - float(t.credit or 0)
            for t in txns
            if not _is_pipeline_txn(t)
        )
...
def detect_invoice_proration_accruals(
    ...
    **Payroll accounts** (account name contains "Pay/Wages" or "Payroll")
    ...
    for acct in gl_data.accounts:
        ...
        if has_range_txns:
            ...
            if _is_metered_utility:
                ...
                # gets double-counted. Uses _real_net_change (excludes only the
                # pipeline's own transactions, not all J-type activity) so a
                # manual JE JLL posts themselves still counts as "real" and
                # correctly suppresses the pipeline's duplicate.
                _non_j_net = _real_net_change(acct)
                _non_j_net = _non_j_net if _non_j_net > 0.01 else 0.0
    ...
    for acct in gl_data.accounts:
        ...
        # Separate the pipeline's OWN entries (prior accruals/reversals) from
        # everything else — actual vendor invoices AND any manual JE JLL (or
        # anyone else) posts, both of which are genuine already-posted activity.
        # Pipeline credits = individual auto-reversal transactions — kept as a
        #   list so each reversal can generate its own accrual line (like elec/gas breakout).
        # Pipeline debits  = prior pipeline JE already in GL → suppress to avoid double-accrual.
        # Using _is_pipeline_txn (not raw J control code) means a JLL-posted
        # manual JE is correctly treated as real activity, not mistaken for
        # the pipeline's own reversal — control code alone can't tell them apart.
        period_debits  = []    # non-pipeline debit amounts — actual vendor invoices (or JLL JEs)
        j_credit_txns  = []    # individual pipeline-credit txns (auto-reversals)
        j_debit_total  = 0.0   # pipeline net debits (prior pipeline JE posted to GL)

        for txn in acct.transactions:
            is_pipeline = _is_pipeline_txn(txn)
            txn_net = (txn.debit or 0) - (txn.credit or 0)

            if is_pipeline:
                if txn_net < 0:
                    j_credit_txns.append(txn)    # credit → pipeline auto-reversal
                elif txn_net > 0:
                    j_debit_total += txn_net      # debit → pipeline JE already in GL
                continue   # pipeline entries never count as vendor invoices

            # Non-pipeline debit → actual vendor invoice or JLL-posted entry
            if txn_net <= 0:
                continue
        ...
        # Collect all non-pipeline debits for this account (real invoices or JLL JEs).
        _invoice_lines = []
        for txn in acct.transactions:
...
def detect_historical_recurring(gl_data, budget_data, period: str = '',
    ...
    for acct in gl_data.accounts:
        ...
        #
        # Strip the pipeline's OWN entries from the partial-coverage signal —
        # a $130K pipeline-debit + $130K pipeline-credit pair (or a JLL-posted
        # manual JE, which _real_net_change also counts as real) shouldn't
        # distort whether a real invoice has already posted this period.
        _kpc_net    = _real_net_change(acct)  # real (non-pipeline) net, incl. JLL manual JEs
...
def build_accrual_entries(nexus_data: list, period: str = '',
    ...
    if tenant_utility_rows:
        ...
        # No suppression here (per-tenant amounts are intentional manual entry,
        # not an auto-detected estimate, so silently dropping them risks losing
        # real data) — but warn if 440500/440700 already has real (non-pipeline)
        # activity this period, since that likely means JLL already posted their
        # own tenant billing and this would duplicate it.
        for _tub_code, _tub_label in (('440500', 'electric'), ('440700', 'gas')):
            _tub_acct = _tub_gl.get(_tub_code)
            if _tub_acct is not None and abs(_real_net_change(_tub_acct)) > 1.0:
                import warnings as _tub_w
                _tub_w.warn(
                    f'Tenant Utility Billing ({_tub_label}, account {_tub_code}) already has '
                    f'real (non-pipeline) GL activity this period — possibly JLL already '
                    f'posted tenant billing. The meter-read JE will still be generated as '
                    f'entered; verify against the GL before uploading to avoid duplicating it.',
                    UserWarning,
                    stacklevel=2,
                )
        ...
        for row in tenant_utility_rows:
            ...
            if elec_amt > 0:
                _post_tub_line(
                    ...
                    tenant_name,
                    f'Accr: Tenant Electric — {tenant_name} (per meter read) '
                    f'${elec_amt:,.2f}',
                    reverse=True,
                )
            ...
            if not _reimb_posted:
                ...
                if _pipeline_reclass < 0.01:
                    ...
                    if abs(_440500_tie_delta_a) >= 0.01:
                        ...
                        _tie_desc_a  = (
                            f'Accr: Tenant Electric — tie to 613115 reclass total '
                            f'(${_pipeline_reclass:,.2f}) '
            ...
            if _elec_by_tenant:
                ...
                for _tenant_name, _tenant_amt in sorted(_elec_by_tenant.items()):
                    _post_tub_line(
                        ...
                        _tenant_name,
                        (f'Accr: Tenant Electric — {_tenant_name} '
                ...
                _post_tub_line(
                    ...
                    '[Receivable Detail]',
                    (f'Accr: Tenant Electric — ${_rec_elec_amt:,.2f} '
            ...
            if budget_data:
                for cand in detect_tenant_utility_billing(gl_data, budget_data):
                    ...
                    _post_tub_line(
                        ...
                        '[Budget Accrual]',
                        f"Accr: Tenant Electric — {cand['description']}",
                        reverse=True,
                    )
                    _tub_accounts.add('440500')
        ...
        if _round(_mode_b_elec_total) == 0:
            ...
            if (_613115_fb_gl is not None
                ...
                _post_tub_line(
                    ...
                    '[JLL Reclass Basis]',
                    (f'Accr: Tenant Electric — derived from JLL 613115 reclass '
                     f'${_fb_amt:,.2f} '
                     f'(DR {TENANT_UTILITY_AR_ACCOUNT} / CR 440500). '
        ...
        if _round(_mode_b_elec_total) == 0 and not _440500_already_posted:
            ...
            if _elec_carryforward > 0:
                _post_tub_line(
                    ...
                    (f'Accr: Tenant Electric — estimated ${_elec_carryforward:,.2f} '
        ...
        if _round(_mode_b_elec_total) == 0 and budget_data:
            ...
            if _613115_ptd_budget >= 1.0:
                _post_tub_line(
                    ...
                    '[Budget Accrual]',
                    (f'Accr: Tenant Electric — ${_613115_ptd_budget:,.2f} '
        ...
        if _round(_mode_b_elec_total) > 0:
            ...
            if not _reimb_b_posted:
                ...
                if _pipeline_reclass_b < 0.01:
                    ...
                    # Always reverses next month — this month's reclass gets
                    # recalculated fresh each period rather than sitting
                    # permanently, same as the underlying 440500 accrual it
                    # ties to. Confirmed with Ryan 2026-08-12.
                    _reclass_rev_flag = -1
                    je_lines.append({
                        'je_number':      _elec_je_id, 'line': 1, 'date': '',
                        'account_code':   ELEC_TENANT_REIMB_ACCOUNT,
...
def generate_etl_csv(je_lines: List[Dict], output_path: str,
    ...
    Args:
        ...
        auto_reverse:  Deprecated — kept for signature compatibility but ignored.
                       ...
                       line posting to 213100 (Accrued Expenses), 213200 (Accrued
                       Interest Payable), or 133110 (AR Billback — Tenant Utility
                       Billing/recovery accrual); 0 otherwise. This ensures only
                       true accrual entries auto-reverse, while prepaid,
                       management fee, and reclassification batches do not.
    ...
    # (Accrued Expenses), 213200 (Accrued Interest Payable), or 133110
    # (Accounts Receivable Billback — the DR leg of every Tenant Utility
    # Billing JE, both electric/440500 and gas/440700, per
    # TENANT_UTILITY_AR_ACCOUNT / _post_tub_line())? Only those batches are
    # true accruals that should auto-reverse next month — interest accruals
    # (801xxx expense routes to 213200 instead of 213100, see _cr_for()) and
    # the tenant recovery accrual (Recovery - Electricity / Recovery - Misc
    # Utilities) need the same always-reverse treatment as any other accrual.
    # All other batches (prepaid releases, mgmt fee, reclasses, etc.) get BM = 0.
    _batches_with_213100: set = {
        line.get('je_number', '')
        for line in je_lines
        if str(line.get('account_code', '') or '').strip() in ('213100', '213200', '133110')
    }
