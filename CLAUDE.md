...
| 4 | Payroll bonus | User-entered annual ÷ 12; Kardin-derived as fallback; compounds monthly until the annual payment (confirmed March), then true-up flagged if it doesn't match |
...
**Layer 4 bonus compounding** (rewritten 2026-09-09): the bonus is paid once a year, in March (confirmed with Ryan), and should build up a growing monthly balance until then — the same compounding-until-a-real-payment-lands pattern Water/Sewer and the Named sub-line mechanism above both use, not a flat monthly amount that resets and reverses every month (which is what this layer did before this fix, and didn't match JLL's own real practice — confirmed on real Jan-Jun 2026 GL data showing a steadily growing bonus balance, not a flat repeating one). Each month without a real bonus payment, this month's `annual/12` share adds onto whatever unmatched balance already reversed in from the prior month. The moment a real bonus payment lands (detected the same bonus-tagged, non-pipeline-transaction way `_real_bonus_net_change` already does), accrual stops for the cycle and the accumulated balance is compared against the real payment — if they differ by more than the greater of $500 or 10%, a `UserWarning` flags it for manual review rather than auto-posting a correction (confirmed with Ryan: flag, don't auto-correct). This is the direct safeguard against Ryan's stated risk: a bonus budgeted for a hire that never happened (or started later than planned) would otherwise silently overstate the accrual all year — instead, the real payment always wins the moment it posts, and any gap is surfaced rather than hidden.
