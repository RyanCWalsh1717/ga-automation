        ...
        with st.spinner("Building accrual entries..."):
            try:
                ...
                if _active_cfg.is_multi_building:
                    ...
                    # Each account's APPROVED allocation, from the Kardin
                    # budget's AllocationName column — the uploaded Kardin
                    # Allocations report is the only authority on how a cost
                    # is split (confirmed with Ryan 2026-09-22); a ratio
                    # merely observed in GL history is never applied here,
                    # only flagged by QC CHECK_9 when real coding has drifted
                    # from the approved allocation.
                    _acct_schedules = {}
                    try:
                        for _krow in (engine_result.parsed.get('kardin_budget') or []):
                            _kcode = str(_krow.get('account_code', '') or '').strip()
                            _kalloc = str(_krow.get('allocation_name', '') or '').strip()
                            if _kcode and _kalloc:
                                _acct_schedules.setdefault(_kcode, _kalloc)
                    except Exception:
                        _acct_schedules = {}

                    all_je_lines = _apply_splits(all_je_lines, _active_cfg,
                                                 account_schedules=_acct_schedules)
