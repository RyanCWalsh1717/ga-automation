        ...
        with st.spinner("Building accrual entries..."):
            try:
                ...
                if _active_cfg.is_multi_building:
                    ...
                    # Real per-account historical split, from the property's
                    # stored 12-Month GL History file, beats a single
                    # property-wide default schedule for any account it
                    # covers (confirmed with Ryan 2026-09-22 against real
                    # Hartwell data: 213200 Accrued Interest genuinely splits
                    # 63/37, not the same ratio as every other account).
                    # Falls back to default_split_schedule for anything not
                    # covered by real history yet.
                    _hist_splits = {}
                    _glh_fname = getattr(_active_cfg, 'gl_history_file', '') or ''
                    _glh_hist_path = _committed_path(_active_cfg.property_code, _glh_fname) if _glh_fname else None
                    if _glh_hist_path:
                        try:
                            from parsers.yardi_gl import parse_gl as _parse_gl_for_splits
                            from gl_history_analyzer import compute_historical_building_splits as _compute_hist_splits
                            _glh_for_splits = _parse_gl_for_splits(_glh_hist_path)
                            _split_buildings = [
                                {'name': b.name, 'yardi_code': b.yardi_code}
                                for b in (_active_cfg.consolidated_buildings or [])
                            ]
                            _hist_splits = _compute_hist_splits(_glh_for_splits, _split_buildings)
                        except Exception:
                            _hist_splits = {}

                    all_je_lines = _apply_splits(all_je_lines, _active_cfg, historical_splits=_hist_splits)
...
with tab4:
    ...
    # ── Building Allocations — Kardin Report auto-extract (outside form) ──────
    st.markdown("### 📐 Building Allocations (Kardin Report)")
    st.caption(
        "Upload Kardin's own Allocations report (rptAllocations PDF) instead of "
        "typing building split percentages in by hand — it lists every named "
        "allocation schedule configured for this property, with a Cost Center "
        "ID per building (confirmed to match the real Yardi cost center)."
    )
    st.warning(
        "⚠️ Kardin is the **budgeting** system, not the AP/accrual system — "
        "confirm with the PM team that these percentages match what **Nexus** "
        "actually uses for cost allocation before relying on them for real "
        "accruals. This upload doesn't perform that cross-check on its own.",
        icon="⚠️",
    )
    _kardin_alloc_upload = st.file_uploader(
        "Kardin Allocations Report (PDF)",
        type=['pdf'],
    ...
    if _kardin_alloc_upload is not None:
        ...
        try:
            ...
            with _ka_tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as _ka_tmp:
                _ka_tmp.write(_kardin_alloc_upload.read())
                _ka_tmp_path = _ka_tmp.name
            from kardin_allocations import parse as _parse_kardin_alloc
            _ka_result = _parse_kardin_alloc(_ka_tmp_path)
            os.remove(_ka_tmp_path)

            if _ka_result._parse_error:
                st.warning(f"⚠️ {_ka_result._parse_error}")
            else:
                _ka_splits = _ka_result.get_multi_way_splits()
                if not _ka_splits:
                    st.info(
                        f"Read {_ka_result.property_display_name or 'this report'} "
